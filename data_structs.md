...
*Last Updated: Current Implementation (Post-Validation-Centralization)*
...
```javascript
{
    ...
    machine_id: string,            // Human-readable ID (auto-generated by BusinessLogicService)
...
- **Required Fields**: `machine_name`, `machine_type`, `work_center`, `department`
- **Numeric Validation**: All numeric fields must be ≥ 0
- **Relationship Validation**: `min_web_width` ≤ `max_web_width`, `min_bag_height` ≤ `max_bag_height`
- **Department Validation**: Machine types must match department
- **Work Center Validation**: Must be `ZANICA` or `BUSTO_GAROLFO`
- **Shifts Validation**: At least one shift must be selected

### Phase Validation
- **Required Fields**: `name`, `department`, `numero_persone`
- **Department Validation**: Must be `STAMPA` or `CONFEZIONAMENTO`
- **People Validation**: `numero_persone` must be ≥ 1
- **Department-Specific**: Printing/packaging parameters must be ≥ 0
- **Content Validation**: `contenuto_fase` required for CONFEZIONAMENTO

### ODP Validation
- **Required Fields**: `odp_number`, `article_code`, `production_lot`, `bag_height`, `bag_width`, `bag_step`, `quantity`, `delivery_date`, `department`, `fase`
- **Format Validation**: 
  - ODP number: `OP000001` format
  - Article code: Must start with `P0` or `ISP0`
  - Production lot: `[A-Z]{2,4}\d{3,6}` format
- **Numeric Validation**: All numeric fields must be ≥ 0, quantity must be > 0
- **Relationship Validation**: `bag_width` ≥ `bag_step`
- **Date Validation**: `delivery_date` must be in the future
- **Department Validation**: Must be `STAMPA` or `CONFEZIONAMENTO`
- **Product Type Validation**: Must be `crema`, `liquido`, or `polveri`
- **Seal Sides Validation**: Must be 3 or 4
...
- **Form Validation**: Comprehensive validation with real-time feedback via ValidationService
...
### Architecture
- **ValidationService**: Centralized validation logic for all entities
- **BusinessLogicService**: Centralized business logic and calculations
- **StorageService**: Data persistence only (no business logic)
- **Managers**: UI logic and user interaction only
